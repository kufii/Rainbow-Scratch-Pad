import { loadImg, randomFloat } from '../util.js';
import { getGradient } from './texgen-util.js';

export const Sheet = function(container, bgCanvas, canvas, uiCanvas, width = 1024, height = 1024) {
	...
	const drawBackground = function() {
		const scale = randomFloat(sheet.bg.scale.min, sheet.bg.scale.max);
		const angle = randomFloat(360);

		const radius = Math.min(sheet.width, sheet.height) / 2;
		const x = randomFloat(radius, sheet.width - radius);
		const y = randomFloat(radius, sheet.height - radius);
		const strength = randomFloat(sheet.bg.twistStrength.min, sheet.bg.twistStrength.max);

		new TG.Texture(sheet.width, sheet.height)
			.add(getGradient(sheet.bg.colors))
